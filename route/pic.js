...
router.get("/show",function (req,res) {
    ...
    file.getContents("./uploads/"+dirName,function (err,pics) {
        if(err){
            res.render("photoShow",{imgs:[]});
        }
        //传递图片数组:图片被点击的文件夹名称
        res.render("photoShow",{imgs:pics,Dirname:dirName});
        res.end();
    });
});
...
router.get("/upload",function (req,res) {
    ...
    file.getContents("./uploads",function (err,files) {
       if(err){
           console.log(err);
           res.render("photoUpload",{status:1,data:"服务器故障,稍后再试"});
           return;
       }
       res.render("photoUpload",{status:0,data:files});
    });
});
...
router.post("/upload",function (req,res) {
    ...
    form.parse(req,function (err,fields,files){
        ...
        res.redirect("/photo/pic/show?dirname="+dirName);
    });
});
